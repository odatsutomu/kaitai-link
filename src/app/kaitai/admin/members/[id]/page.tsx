...
function RadarChart({ radar }: { radar: Record<string, number> }) {
  ...
  const n = 5;
  const cx = 120, cy = 115, r = 80, labelR = 100;
  const angles = Array.from({ length: n }, (_, i) => (i * 2 * Math.PI) / n - Math.PI / 2);
  const toXY = (ang: number, radius: number) => ({ x: cx + radius * Math.cos(ang), y: cy + radius * Math.sin(ang) });
  const gridLevels = [0.25, 0.5, 0.75, 1.0];
  const gridPolygons = gridLevels.map(scale => angles.map(a => `${toXY(a, r * scale).x},${toXY(a, r * scale).y}`).join(" "));
  const dataPts = vals.map((v, i) => { const { x, y } = toXY(angles[i], r * v / 100); return `${x},${y}`; }).join(" ");
  return (
    <svg viewBox="0 0 240 230" className="w-full">
      {gridPolygons.map((pts, i) => <polygon key={i} points={pts} fill="none" stroke="#2D3E54" strokeWidth={i === 3 ? "0.8" : "0.5"} />)}
      {angles.map((a, i) => { const tip = toXY(a, r); return <line key={i} x1={cx} y1={cy} x2={tip.x} y2={tip.y} stroke="#2D3E54" strokeWidth="0.5" />; })}
      <polygon points={dataPts} fill="rgba(249,115,22,0.15)" stroke="#F97316" strokeWidth="1.8" strokeLinejoin="round" />
      {vals.map((v, i) => { const { x, y } = toXY(angles[i], r * v / 100); return <circle key={i} cx={x} cy={y} r="3.5" fill="#F97316" stroke="#0F1928" strokeWidth="1" />; })}
      {vals.map((v, i) => { const offset = 12; const { x, y } = toXY(angles[i], r * v / 100 + offset); return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="8" fill="#F97316" fontWeight="bold">{v}</text>; })}
      {labels.map((label, i) => { const { x, y } = toXY(angles[i], labelR); return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="9" fill="#94A3B8">{label}</text>; })}
    </svg>
  );
}
...
function AttendanceCalendar({ calendar }: { calendar: string[] }) {
  ...
  const statusColor = (s: string) => {
    ...
    return { bg: "#0F1928", border: "transparent", text: "#1A2535" };
  };
  return (
    <div>
      <div className="grid grid-cols-7 gap-1 mb-1">
        {days.map((d, i) => <div key={d} className="text-center py-0.5" style={{ fontSize: 12, fontWeight: 700, color: i >= 5 ? "#F87171" : "#64748B" }}>{d}</div>)}
      </div>
      <div className="grid grid-cols-7 gap-1">
        ...
        {calendar.map((status, i) => {
          ...
          return (
            <div key={i} className="rounded-lg flex flex-col items-center justify-center" style={{ aspectRatio: "1", background: c.bg, border: `1px solid ${c.border}`, opacity: status === "未来" ? 0.3 : 1 }}>
              <span style={{ fontSize: 12, fontWeight: 700, color: colIdx >= 5 ? (status === "休日" ? "#F87171" : c.text) : c.text }}>{i + 1}</span>
              {status === "遅刻" && <span style={{ fontSize: 10, color: "#FBBF24" }}>遅</span>}
              {status === "欠勤" && <span style={{ fontSize: 10, color: "#EF4444" }}>欠</span>}
            </div>
          );
        })}
      </div>
...
function TroubleCard({ t, onScore }: { t: TroubleRecord; onScore: (id: string, score: 1 | 2 | 3, memo: string) => void }) {
  const [open, setOpen] = useState(false);
  const [memo, setMemo] = useState(t.adminMemo ?? "");
  const [saving, setSaving] = useState(false);
  const typeColor = { "近隣クレーム": "#FBBF24", "埋設物": "#60A5FA", "事故": "#F87171", "機材故障": "#94A3B8", "ルール違反": "#F87171", "その他": "#94A3B8" }[t.type] ?? "#94A3B8";
  const handleScore = (score: 1 | 2 | 3) => { setSaving(true); setTimeout(() => { onScore(t.id, score, memo); setSaving(false); setOpen(false); }, 300); };
      ...
      <button className="w-full text-left px-4 py-3 flex items-start gap-3" onClick={() => setOpen(o => !o)}>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-0.5 flex-wrap">
            ...
            <span style={{ fontSize: 13, color: "#64748B" }}>{t.date}・{t.site}</span>
          </div>
          <p style={{ fontSize: 13, color: "#94A3B8" }} className="line-clamp-2">{t.detail}</p>
          {cfg && !open && (
            <div className="flex items-center gap-1 mt-1">
              <cfg.icon size={12} style={{ color: cfg.color }} />
              <span style={{ fontSize: 12, fontWeight: 700, color: cfg.color }}>管理者評価: {cfg.label}</span>
            </div>
          )}
        </div>
        {open ? <ChevronUp size={14} style={{ color: "#475569" }} className="flex-shrink-0 mt-1" /> : <ChevronDown size={14} style={{ color: "#475569" }} className="flex-shrink-0 mt-1" />}
      </button>
      {open && (
        <div className="px-4 pb-4 flex flex-col gap-3" style={{ borderTop: "1px solid #0F1928" }}>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: "#64748B" }}>管理者評価（3段階）</p>
            <div className="flex gap-2">
              {([1, 2, 3] as const).map(score => {
                const c = SCORE_CONFIG[score];
                const active = t.adminScore === score;
                return (
                  <button key={score} onClick={() => handleScore(score)} disabled={saving}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl transition-all"
                    style={{ fontSize: 13, fontWeight: 700, background: active ? c.bg : "rgba(15,25,40,0.5)", color: active ? c.color : "#475569", border: active ? `1px solid ${c.color}30` : "1px solid #2D3E54" }}>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: "#64748B" }}>管理者メモ</p>
            <textarea value={memo} onChange={e => setMemo(e.target.value)} rows={2} placeholder="対応内容・指示事項など…"
          ...
          {t.adminMemo && (
            <div className="px-3 py-2 rounded-xl" style={{ background: "rgba(249,115,22,0.06)", border: "1px solid rgba(249,115,22,0.12)" }}>
              <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 2, color: "#F97316" }}>保存済メモ</p>
              <p style={{ fontSize: 13, color: "#94A3B8" }}>{t.adminMemo}</p>
            </div>
          )}
        </div>
      )}
...
function exportCSV(memberName: string, stats: typeof MEMBER_STATS[0]) {
  const rows: (string | number)[][] = [
    ["項目", "値"], ["氏名", memberName],
    ["当月出勤日数", stats.workDays], ["遅刻回数", stats.lateDays],
    ["欠勤日数", stats.absentDays], ["合計勤務時間(h)", stats.totalHours],
    ["平均残業時間(h)", stats.avgOvertime], ["出勤率(%)", stats.attendancePct],
    ["効率スコア(%差)", `${stats.efficiencyDelta > 0 ? "+" : ""}${stats.efficiencyDelta}%`],
    ["ルール違反件数", stats.ruleViolations], ["トラブル対応件数", stats.troubles.length],
    [], ["レーダー評価"],
    ["勤怠率", stats.radar.attendance], ["安全性", stats.radar.safety],
    ["技術力", stats.radar.skill], ["コミュニケーション", stats.radar.communication],
  ...
  a.href = url; a.download = `${memberName}_月次レポート_2026-04.csv`; a.click();
...
export default function AdminMemberDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [statsState, setStatsState] = useState(() => MEMBER_STATS.find(s => s.memberId === id) ?? MEMBER_STATS[0]);
  ...
  if (!member) return <div className="flex items-center justify-center min-h-screen" style={{ color: "#64748B" }}>メンバーが見つかりません</div>;
  ...
  const handleTroubleScore = (troubleId: string, score: 1 | 2 | 3, memo: string) => {
    setStatsState(prev => ({ ...prev, troubles: prev.troubles.map(t => t.id === troubleId ? { ...t, adminScore: score, adminMemo: memo } : t) }));
  };

  const AVAILABLE_TAGS = ["#効率的", "#リーダーシップ", "#安全模範", "#要注意", "#ルール違反", "#成長中", "#冷静対応"];

  return (
    <div className="py-6 pb-8 flex flex-col gap-5">

      {/* ── Hero ── */}
      <section className="rounded-xl p-5" style={{ background: "linear-gradient(160deg, #0F1928 0%, #1A2535 100%)", border: "1px solid #2D3E54" }}>
        <Link href="/kaitai/admin/members" className="inline-flex items-center gap-1.5 mb-4 text-sm" style={{ color: "#64748B" }}>
          <ArrowLeft size={15} /> 従業員管理
        </Link>
        <div className="flex items-start gap-4 mb-4">
          ...
          <button onClick={() => exportCSV(member.name, s)}
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: "#F97316" }}>CSV</span>
          </button>
        </div>
        ...
        <div>
          <div className="flex justify-between mb-1" style={{ fontSize: 13, color: "#64748B" }}>
          ...
          <div className="flex justify-between mt-1" style={{ fontSize: 12, color: "#2D3E54" }}>
      ...
      <div className="py-1">
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: "#1A2535" }}>
          {(["基本情報", "勤怠", "パフォーマンス", "評価"] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)} className="flex-1 py-2 rounded-xl transition-all"
              style={{ fontSize: 14, fontWeight: 700, ...(tab === t ? { background: "rgba(249,115,22,0.15)", color: "#F97316" } : { color: "#64748B" }) }}>
              {t}
            </button>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        {tab === "基本情報" && (
          <>
            ...
            <section>
              <SectionLabel>基本情報（管理者専用）</SectionLabel>
              <Card>
                {[
                ...
                <div className="flex items-start gap-3 px-4" style={{ paddingTop: 16, paddingBottom: 16, minHeight: 64, borderTop: "1px solid #0F1928" }}>
                  ...
                  <div>
                    <p style={{ fontSize: 13, color: "#64748B" }}>住所</p>
                ...
                <div className="flex items-start gap-3 px-4" style={{ paddingTop: 16, paddingBottom: 16, minHeight: 64, borderTop: "1px solid #0F1928" }}>
                  ...
                  <div>
                    <p style={{ fontSize: 13, color: "#64748B" }}>緊急連絡先</p>
            ...
            <section>
              <div className="grid grid-cols-3 gap-2">
                <Card className="p-4 text-center">
                  ...
                  <p style={{ fontSize: 13, marginTop: 2, color: "#64748B" }}>累計現場数</p>
                </Card>
                <Card className="p-4 text-center">
                  <p style={{ fontSize: 18, fontWeight: 700, color: lvl.color }}>{yrs}年</p>
                  <p style={{ fontSize: 13, marginTop: 2, color: "#64748B" }}>累計経験年数</p>
                </Card>
                <Card className="p-4 text-center">
                  <p style={{ fontSize: 18, fontWeight: 700, color: "#4ADE80" }}>¥{member.dayRate.toLocaleString()}</p>
                  <p style={{ fontSize: 13, marginTop: 2, color: "#64748B" }}>日当</p>
                </Card>
              </div>
            </section>
            ...
            <section>
              ...
              {s.siteEvals.map((e, i) => (
                <Card key={i} className="px-4 flex items-center gap-3 mb-2" style={{ paddingTop: 14, paddingBottom: 14, minHeight: 64 }}>
                  ...
                  <div className="flex-1">
                    <p style={{ fontSize: 15, fontWeight: 600, color: "#F1F5F9" }}>{e.site}</p>
                    <p style={{ fontSize: 13, color: "#64748B" }}>{e.date}</p>
                  </div>
                  {e.role === "責任者" && <span style={{ fontSize: 13, fontWeight: 700, padding: "3px 10px", borderRadius: 20, background: "rgba(251,191,36,0.12)", color: "#FBBF24" }}>責任者</span>}
                </Card>
              ))}
            </section>
          </>
          ...
          <>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {[
                  ...
                  <Card key={label} className="p-4">
                    <p style={{ fontSize: 13, marginBottom: 4, color: "#64748B" }}>{label}</p>
            ...
            <section>
              <div className="flex justify-between mb-1">
                <span style={{ fontSize: 13, color: "#64748B" }}>出勤率</span>
                <span style={{ fontSize: 13, fontWeight: 700, color: attColor }}>{s.attendancePct}%</span>
              </div>
              <div className="h-3 rounded-full overflow-hidden" style={{ background: "#0F1928" }}>
            ...
            <section>
              ...
              <Card className="p-4"><AttendanceCalendar calendar={s.calendar} /></Card>
            </section>
            {s.lateDays > 0 && (
              <div className="rounded-2xl px-4 py-3 flex items-center gap-3" style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: "#94A3B8" }}>現場端末の開始打刻が予定時刻を超過した記録</p>
                </div>
              </div>
            )}
          </>
            ...
            <section>
              ...
              <Card className="p-4">
                ...
                <div className="grid grid-cols-5 gap-1 mt-2">
                  {[{ label: "勤怠", v: s.radar.attendance }, { label: "安全", v: s.radar.safety }, { label: "技術", v: s.radar.skill }, { label: "コミュ", v: s.radar.communication }, { label: "効率", v: s.radar.efficiency }].map(({ label, v }) => (
            ...
            <section>
              ...
              <Card className="p-4">
                <div className="flex items-center gap-3">
                  <div className="w-14 h-14 rounded-2xl flex-shrink-0 flex flex-col items-center justify-center" style={{ background: s.efficiencyDelta >= 0 ? "rgba(74,222,128,0.1)" : "rgba(239,68,68,0.1)" }}>
                    {s.efficiencyDelta >= 0 ? <TrendingUp size={22} style={{ color: "#4ADE80" }} /> : <TrendingDown size={22} style={{ color: "#F87171" }} />}
                  </div>
                  <div className="flex-1">
                    <p style={{ fontSize: 32, fontWeight: 800, color: effColor, lineHeight: 1 }}>{s.efficiencyDelta > 0 ? "+" : ""}{s.efficiencyDelta}%</p>
                    <p style={{ fontSize: 13, color: "#64748B", marginTop: 4 }}>標準工数との比較（マイナスが速い）</p>
                  </div>
                </div>
              </Card>
            </section>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Card className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p style={{ fontSize: 13, fontWeight: 700, color: "#64748B" }}>ルール違反</p>
                  </div>
                  <p style={{ fontSize: 26, fontWeight: 800, color: s.ruleViolations > 0 ? "#F87171" : "#4ADE80", lineHeight: 1 }}>{s.ruleViolations}件</p>
                </Card>
                <Card className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <CheckCircle size={16} style={{ color: "#4ADE80" }} />
                    <p style={{ fontSize: 13, fontWeight: 700, color: "#64748B" }}>ポジティブ</p>
                  </div>
                  <p style={{ fontSize: 26, fontWeight: 800, color: "#4ADE80", lineHeight: 1 }}>{s.positiveFeedback.length}件</p>
                </Card>
              </div>
            </section>
            <section>
              ...
              {s.troubles.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {s.troubles.map(t => <TroubleCard key={t.id} t={t} onScore={handleTroubleScore} />)}
                </div>
              )}
            </section>
          </>
        ...
        {tab === "評価" && (
          <>
                ...
                {s.siteEvals.map((e, i) => (
                  <Card key={i} className="p-4">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      {e.role === "責任者" && <span style={{ fontSize: 12, fontWeight: 700, padding: "2px 8px", borderRadius: 20, marginLeft: "auto", background: "rgba(251,191,36,0.12)", color: "#FBBF24" }}>責任者</span>}
                    </div>
                    ...
                    {e.memo && (
                      <div className="flex items-start gap-2 pt-2" style={{ borderTop: "1px solid #0F1928" }}>
                        ...
                        <p style={{ fontSize: 13, color: "#94A3B8" }}>{e.memo}</p>
                      </div>
                    )}
                  </Card>
                ))}
            ...
            <section>
              ...
              <Card className="p-4">
                <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: "#64748B" }}>タグを選択</p>
                <div className="flex flex-wrap gap-2 mb-3">
                  {AVAILABLE_TAGS.map(tag => {
                    const active = newEvalTags.includes(tag);
                    const ts = tagStyle(tag);
                    return (
                      <button key={tag} onClick={() => setNewEvalTags(prev => active ? prev.filter(t => t !== tag) : [...prev, tag])}
                        style={{ fontSize: 13, fontWeight: 700, padding: "4px 12px", borderRadius: 20, transition: "all 0.15s", ...(active ? { background: ts.bg, color: ts.color, border: `1px solid ${ts.color}40` } : { background: "#0F1928", color: "#475569", border: "1px solid #2D3E54" }) }}>
                        {tag}
                      </button>
                    );
                  })}
                </div>
                <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: "#64748B" }}>メモ</p>
                <textarea value={newEvalMemo} onChange={e => setNewEvalMemo(e.target.value)} rows={3} placeholder="今回の仕事ぶりについてメモ…"
                  className="w-full rounded-xl px-3 py-2 resize-none outline-none"
                  style={{ fontSize: 14, background: "#0F1928", color: "#F1F5F9", border: "1px solid #2D3E54" }} />
                <button
                  onClick={() => {
                    if (newEvalTags.length === 0 && !newEvalMemo) return;
                    setStatsState(prev => ({ ...prev, siteEvals: [{ date: "4月2日", site: "（最新現場）", role: "作業員", tags: newEvalTags, memo: newEvalMemo }, ...prev.siteEvals] }));
                    setNewEvalTags([]); setNewEvalMemo("");
                  }}
                  className="w-full mt-3 py-2.5 rounded-xl font-bold transition-all active:scale-[0.98]"
                  style={{ fontSize: 14, background: newEvalTags.length > 0 || newEvalMemo ? "linear-gradient(135deg, #F97316, #FBBF24)" : "#1A2535", color: newEvalTags.length > 0 || newEvalMemo ? "#0F1928" : "#475569" }}
                >
                  評価を保存
                </button>
              </Card>
