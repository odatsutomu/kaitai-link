...
function RadarChart({ radar }: { radar: Record<string, number> }) {
  ...
  return (
    <svg viewBox="0 0 240 230" className="w-full">
      ...
      {vals.map((v, i) => { const { x, y } = toXY(angles[i], r * v / 100); return <circle key={i} cx={x} cy={y} r="3.5" fill={T.primary} stroke="#0F1928" strokeWidth="1" />; })}
      {vals.map((v, i) => { const offset = 12; const { x, y } = toXY(angles[i], r * v / 100 + offset); return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="8" fill={T.primary} fontWeight="bold">{v}</text>; })}
      {labels.map((label, i) => { const { x, y } = toXY(angles[i], labelR); return <text key={i} x={x} y={y} textAnchor="middle" dominantBaseline="middle" fontSize="9" fill={T.muted}>{label}</text>; })}
    </svg>
  );
}
  ...
  return (
    <div>
      <div className="grid grid-cols-7 gap-1 mb-1">
        {days.map((d, i) => <div key={d} className="text-center py-0.5" style={{ fontSize: 12, fontWeight: 700, color: i >= 5 ? "#F87171" : T.sub }}>{d}</div>)}
      </div>
        ...
        {calendar.map((status, i) => {
          ...
          return (
            <div key={i} className="rounded-lg flex flex-col items-center justify-center" style={{ aspectRatio: "1", background: c.bg, border: `1px solid ${c.border}`, opacity: status === "未来" ? 0.3 : 1 }}>
              ...
              {status === "遅刻" && <span style={{ fontSize: 10, color: "#92400E" }}>遅</span>}
...
function TroubleCard({ t, onScore }: { t: TroubleRecord; onScore: (id: string, score: 1 | 2 | 3, memo: string) => void }) {
  ...
  const typeColor = { "近隣クレーム": "#92400E", "埋設物": "#60A5FA", "事故": "#F87171", "機材故障": T.muted, "ルール違反": "#F87171", "その他": T.muted }[t.type] ?? T.muted;
    ...
    <div className="rounded-2xl overflow-hidden" style={{ background: "#1A2535", border: "1px solid #2D3E54" }}>
      ...
      {open && (
        <div className="px-4 pb-4 flex flex-col gap-3" style={{ borderTop: "1px solid #0F1928" }}>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: T.sub }}>管理者評価（3段階）</p>
          ...
          <div>
            <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: T.sub }}>管理者メモ</p>
          ...
          {t.adminMemo && (
            <div className="px-3 py-2 rounded-xl" style={{ background: T.primaryLt, border: `1px solid ${T.primaryLt}` }}>
              <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 2, color: T.primary }}>保存済メモ</p>
              <p style={{ fontSize: 13, color: T.muted }}>{t.adminMemo}</p>
            </div>
          )}
        </div>
      )}
...
export default function AdminMemberDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!member) return <div className="flex items-center justify-center min-h-screen" style={{ color: T.sub }}>メンバーが見つかりません</div>;
  ...
  return (
    <div className="py-6 pb-8 flex flex-col gap-5">
      ...
      <section className="rounded-xl p-5" style={{ background: "linear-gradient(160deg, #0F1928 0%, #1A2535 100%)", border: "1px solid #2D3E54" }}>
        <Link href="/kaitai/admin/members" className="inline-flex items-center gap-1.5 mb-4 text-sm" style={{ color: T.sub }}>
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <button onClick={() => exportCSV(member.name, s)}
            ...
            <span style={{ fontSize: 12, fontWeight: 700, color: T.primary }}>CSV</span>
          </button>
        </div>
        ...
        <div>
          <div className="flex justify-between mb-1" style={{ fontSize: 13, color: T.sub }}>
        ...
        <div className="flex gap-1 p-1 rounded-2xl" style={{ background: "#1A2535" }}>
          {(["基本情報", "勤怠", "パフォーマンス", "評価"] as Tab[]).map(t => (
            <button key={t} onClick={() => setTab(t)} className="flex-1 py-2 rounded-xl transition-all"
              style={{ fontSize: 14, fontWeight: 700, ...(tab === t ? { background: T.primaryMd, color: T.primary } : { color: T.sub }) }}>
              {t}
            </button>
          ))}
        </div>
              ...
              <Card>
                {[
                  ...
                  <div key={label} className="flex items-start gap-3 px-4" style={{ paddingTop: 16, paddingBottom: 16, minHeight: 64, borderTop: i > 0 ? "1px solid #0F1928" : undefined }}>
                    ...
                    <div>
                      <p style={{ fontSize: 13, color: T.sub }}>{label}</p>
                ...
                <div className="flex items-start gap-3 px-4" style={{ paddingTop: 16, paddingBottom: 16, minHeight: 64, borderTop: "1px solid #0F1928" }}>
                  ...
                  <div>
                    <p style={{ fontSize: 13, color: T.sub }}>住所</p>
                    <p style={{ fontSize: 15, fontWeight: 500, marginTop: 2, color: T.bg }}>{member.address}</p>
                  </div>
                </div>
                <div className="flex items-start gap-3 px-4" style={{ paddingTop: 16, paddingBottom: 16, minHeight: 64, borderTop: "1px solid #0F1928" }}>
                  <Phone size={16} style={{ color: "#475569" }} className="flex-shrink-0 mt-0.5" />
                  <div>
                    <p style={{ fontSize: 13, color: T.sub }}>緊急連絡先</p>
            ...
            <section>
              <div className="grid grid-cols-3 gap-2">
                <Card className="p-4 text-center">
                  ...
                  <p style={{ fontSize: 13, marginTop: 2, color: T.sub }}>累計現場数</p>
                </Card>
                <Card className="p-4 text-center">
                  <p style={{ fontSize: 18, fontWeight: 700, color: lvl.color }}>{yrs}年</p>
                  <p style={{ fontSize: 13, marginTop: 2, color: T.sub }}>累計経験年数</p>
                </Card>
                <Card className="p-4 text-center">
                  <p style={{ fontSize: 18, fontWeight: 700, color: "#4ADE80" }}>¥{member.dayRate.toLocaleString()}</p>
                  <p style={{ fontSize: 13, marginTop: 2, color: T.sub }}>日当</p>
                </Card>
              </div>
            </section>
          ...
          <>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {[
                  ...
                  <Card key={label} className="p-4">
                    <p style={{ fontSize: 13, marginBottom: 4, color: T.sub }}>{label}</p>
            ...
            <section>
              <div className="flex justify-between mb-1">
                <span style={{ fontSize: 13, color: T.sub }}>出勤率</span>
            ...
            {s.lateDays > 0 && (
              <div className="rounded-2xl px-4 py-3 flex items-center gap-3" style={{ background: "rgba(251,191,36,0.08)", border: "1px solid rgba(251,191,36,0.2)" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: T.muted }}>現場端末の開始打刻が予定時刻を超過した記録</p>
                </div>
              </div>
            )}
          </>
                ...
                <div className="grid grid-cols-5 gap-1 mt-2">
                  {[{ label: "勤怠", v: s.radar.attendance }, { label: "安全", v: s.radar.safety }, { label: "技術", v: s.radar.skill }, { label: "コミュ", v: s.radar.communication }, { label: "効率", v: s.radar.efficiency }].map(({ label, v }) => (
                    <div key={label} className="text-center">
                      ...
                      <p style={{ fontSize: 12, color: T.sub }}>{label}</p>
                    </div>
                  ))}
                </div>
              </Card>
              ...
              <Card className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p style={{ fontSize: 13, color: T.sub, marginTop: 4 }}>標準工数との比較（マイナスが速い）</p>
                  </div>
                </div>
              </Card>
            </section>
              ...
              <div className="grid grid-cols-2 gap-2">
                <Card className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p style={{ fontSize: 13, fontWeight: 700, color: T.sub }}>ルール違反</p>
                  </div>
                ...
                <Card className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <p style={{ fontSize: 13, fontWeight: 700, color: T.sub }}>ポジティブ</p>
                  </div>
        ...
        {tab === "評価" && (
          <>
                  ...
                  <Card key={i} className="p-4">
                    ...
                    {e.memo && (
                      <div className="flex items-start gap-2 pt-2" style={{ borderTop: "1px solid #0F1928" }}>
                        ...
                        <p style={{ fontSize: 13, color: T.muted }}>{e.memo}</p>
                      </div>
                    )}
                  </Card>
                ))}
            ...
            <section>
              ...
              <Card className="p-4">
                <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 8, color: T.sub }}>タグを選択</p>
                ...
                <p style={{ fontSize: 13, fontWeight: 700, marginBottom: 4, color: T.sub }}>メモ</p>
                ...
                <button
                  ...
                  style={{ fontSize: 14, background: newEvalTags.length > 0 || newEvalMemo ? "linear-gradient(135deg, #B45309, #92400E)" : "#1A2535", color: newEvalTags.length > 0 || newEvalMemo ? "#0F1928" : "#475569" }}
                >
                  評価を保存
                </button>
              </Card>
